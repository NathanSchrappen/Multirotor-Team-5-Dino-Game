...
def calculate_jump(enter : float, exit : float) -> float:
    ...
    jumpTime = hitTime - 0.5 #edit second number accordingly as this is the delay it takes the dino to jump
...
while event.is_set() == 0:
    ...
    if (avg > 40):
        ...
        threading.Timer(calculate_jump(time_stack1.pop(0),time_stack2.pop(0)), lambda: keyboard.press_and_release("Space")).start()
